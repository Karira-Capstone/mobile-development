...
class ProyekMainFragment : Fragment() {
    ...
    val proyekMainViewModel: ProyekMainViewModel by viewModels {
...
private fun ProyekMainApp(proyekMainViewModel: ProyekMainViewModel, view: View, id: Int) {
    ...
    val userDataStore =
        proyekMainViewModel.userDataStore.collectAsState(initial = UserDataStore("", "ssss"))

    proyekMainViewModel.uiState
        .collectAsState(initial = UiState.Loading).value.let { uiState ->
            when (uiState) {
                is UiState.Loading -> {
                    proyekMainViewModel.getProjectsByCategory(id+1)
                    Column(
                                ...
                                Column(
                                    ...
                                    Row(
                                            ...
                                            Button(
                                                onClick = {
                                                    view.findNavController()
                                                        .navigate(R.id.action_proyekMainFragment_to_rekomendasiFragment)
                                                },
                                        ...
                                        OutlinedButton(
                                            onClick = {
                                                view.findNavController()
                                                    .navigate(R.id.action_proyekMainFragment_to_proyekSearchFragment)
                                            },
                                            shape = RoundedCornerShape(16),
                                            border = BorderStroke(
                                                1.dp,
                                            ...
                                            modifier = Modifier
                                                .padding(start = if (userDataStore.value.role == "WORKER") 4.dp else 0.dp)
                                                .fillMaxWidth()
                                                .weight(1f)
                                        ) {
                                ...
                                ItemCard(
                                    ...
                                    onClick = {
                                        ...
                                        view.findNavController().navigate(
                                            R.id.action_proyekMainFragment_to_proyekDetailFragment,
                                            bundle
                                        )
                                    })
                            }
...
private fun Preview() {
    val context = LocalContext.current
    ProyekMainApp(
        proyekMainViewModel = ProyekMainViewModel(Injection.provideProyekRepostory(context)),
        View(context),
        1
    )
}