...
class ProyekKuFragment : Fragment() {
    ...
    val proyekKuViewModel: ProyekKuViewModel by viewModels { ViewModelFactory.getInstance(requireContext()) }
...
private fun ProyekKuApp(proyekKuViewModel: ProyekKuViewModel, view: View) {
    ...
    val userDataStore = proyekKuViewModel.userDataStore.collectAsState(initial = UserDataStore("", "ssss"))
                        ...
                        item {
                            Column(modifier = Modifier) {
                                Column(modifier = Modifier.padding(start = 16.dp, end = 16.dp, top = 48.dp, bottom = 24.dp)) {
                                    TitleSection(
                                        title = stringResource(id = R.string.proyek_title, "", "ku"),
                                    ...
                                    Row(
                                        ...
                                        OutlinedButton(
                                            onClick = {
                                                val bundle = Bundle()
                                                bundle.putString(ProyekDetailFragment.EXTRA_ID, "null")
                                                view.findNavController().navigate(R.id.action_proyekKuFragment_to_proyekBuatFragment, bundle)
                                            },
                        ...
                        items(data, key = { it.id.toString() }) { project ->
                            ItemCard(
                                ...
                                onClick = {
                                    ...
                                    view.findNavController().navigate(R.id.action_proyekKuFragment_to_proyekDetailFragment, bundle)
                                })
                        }
                    }
                }
...
private fun Preview(){
    val context = LocalContext.current
    ProyekKuApp(proyekKuViewModel = ProyekKuViewModel(Injection.provideProyekRepostory(context)), View(context))
}